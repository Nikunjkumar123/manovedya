    ...
    useEffect(() => {
        const fetchCompanies = async () => {
            try {
                ...
                toast.error(error.response ? error.response.data.message : 'Error fetching reference companies');
            } finally {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmed.isConfirmed) {
            try {
                ...
                toast.error(error.response ? error.response.data.message : 'Error deleting reference company');
            }
        }
    };
    ...
    return (
        <>
            ...
            <div className="bread">
                ...
                <div className="links">
                    <Link to="/add-ref-company" className="add-new">Add New <i className="fa-solid fa-plus"></i></Link>
                </div>
            </div>
                ...
                <table className="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan="4" className="text-center">No reference companies available</td>
                            </tr>
                        ) : (
                            companies.map((company, index) => (
                                <tr key={company._id}>
                                    <th scope="row">{index + 1}</th>
                                    <td>{company.refCompanyName}</td> {/* Display reference company name */}
                                    <td>
                                        <Link to={`/edit-ref-company/${company._id}`} className="bt edit">Edit <i className="fa-solid fa-pen-to-square"></i></Link>
                                    </td>
                                    <td>
                                        <button onClick={() => handleDelete(company._id)} className="bt delete">Delete <i className="fa-solid fa-trash"></i></button>
                                    </td>
                                </tr>
                            ))
                        )}
