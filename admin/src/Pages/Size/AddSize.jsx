...
const AddSize = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:8000/api/create-size', formData); // Adjust the URL as needed
            toast.success(response.data.message);
            navigate('/all-size'); // Redirect to the all sizes page
        } catch (error) {
            toast.error(error.response ? error.response.data.message : 'Error adding size');
        } finally {
    ...
    return (
        <>
            ...
            <div className="bread">
                ...
                <div className="links">
                    <Link to="/all-size" className="add-new">Back <i className="fa-regular fa-circle-left"></i></Link>
                </div>
            </div>
            ...
            <div className="d-form">
                <form className="row g-3" onSubmit={handleSubmit}>
                    <div className="col-md-6">
                        <label htmlFor="sizeweight" className="form-label">Size Weight</label>
                        <input
                            type="text"
                    ...
                    <div className="col-12 text-center">
                        <button type="submit" disabled={isLoading} className={`${isLoading ? 'not-allowed' : 'allowed'}`}>
                            {isLoading ? "Please Wait..." : "Add Size"} {/* Updated button text */}
                        </button>
                    </div>
                </form>
            </div>
