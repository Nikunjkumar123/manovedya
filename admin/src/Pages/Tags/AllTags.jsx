...
const AllTags = () => {
    ...
    const fetchTags = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get('http://localhost:8000/api/get-tags');
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(`http://localhost:8000/api/delete-tags/${id}`);
                        ...
                        {isLoading ? (
                            ...
                            tags.length > 0 ? tags.map((tag, index) => (
                                <tr key={tag._id}>
                                    ...
                                    <td>{tag.tagName}</td>
                                    <td>
                                        <div className="circle-color" style={{ backgroundColor: tag.tagColor }}></div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <button onClick={() => handleDelete(tag._id)} className="bt delete">
                                            Delete <i className="fa-solid fa-trash"></i>
                                        </button>
                                    </td>
                                </tr>
                            )) : (
                                <tr><td colSpan="5" className="text-center">No tags found</td></tr>
                            )
                        )}
                    </tbody>
                </table>
